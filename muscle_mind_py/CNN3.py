...
class CNNnet(torch.nn.Module):
    def __init__(self):
        ...
        self.soft2 = torch.nn.Softplus()
        self.mlp2_3 = torch.nn.Linear(100, 3)
        self.soft3 = torch.nn.Softplus()
    ...
    def forward(self, x):
        ...
        x = self.mlp1_1(x1.view(x.size(0), -1))
        x = self.dropout(x)
        x = self.mlp2_1(x)
        x = self.soft1(x)

        x2 = self.mlp1_2(x1.view(x.size(0), -1))
        x2 = self.dropout(x2)
        x2 = self.mlp2_2(x2)
        x2 = self.soft2(x2)

        x1_max = torch.max(x, dim=1).indices
        x2_max = torch.max(x2, dim=1).indices
        mask = torch.zeros((x2.shape[0], x.shape[1]+x2.shape[1]))
        for i in range(x2.shape[0]):
            mask[i, x1_max[i]] = 1
            mask[i, x2_max[i] + x.shape[1]] = 1

        mask = self.mlp3_3(mask)
        x3 = self.mlp1_3(x1.view(x.size(0), -1))
        x3 = self.dropout(x3)
        x3 = self.mlp2_3(x3)
        x3 = x3 * mask
        x3 = self.mlp3_3(x3)
