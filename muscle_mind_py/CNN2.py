...
def read_data(train_path, ele1, lable_path, ele2, lable_path2, ele3, isTrain):
...
def test(title, X, Y):
    test_output1, test_output2 = model(X)
    ...
    print('{} epoch {} =====>> acc1: {:.4f}<<=====>> acc2: {:.4f}<<=====>> '.format(title, epoch, acc1, acc2))


class CrossStich(nn.Module):
    def __init__(self):
        super(CrossStich, self).__init__()
        self.params = nn.Parameter(torch.tensor([0.5, 0.5, 0.5, 0.5]).resize(2, 2), requires_grad=True)

    def forward(self, x1, x2):
        # self.params[:, 0] = self.params[:, 0] / torch.sum(self.params[:, 0])
        # self.params[:, 1] = self.params[:, 1] / torch.sum(self.params[:, 1])
        output1 = x1 * self.params[0, 0] + x2 * self.params[1, 0]
        output2 = x1 * self.params[0, 1] + x2 * self.params[1, 1]
...
class CNNnet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x1, x2 = self.cross1(x1, x2)

        x1 = self.conv2_1(x1)
        x2 = self.conv2_2(x2)

        x1, x2 = self.cross2(x1, x2)

        x1 = self.conv3_1(x1)
        x2 = self.conv3_2(x2)

        x1, x2 = self.cross3(x1, x2)
...
X_train, y_train, train_loader = read_data("dataset/train_data_2d_.mat", "train_data_2d",
                                           ...
                                           "action_train_label",
                                           "dataset/mindeffort_train_label1.mat", "mindeffort_train_label", True)
X_test1, y_test1, test_loader1 = read_data("dataset/test_data_2d_.mat", "test_data_2d",
                                           "dataset/action_test_label1.mat",
                                           "action_test_label",
                                           "dataset/mindeffort_test_label1.mat", "mindeffort_test_label", False)
...
for epoch in range(EPOCH):
    ...
    for iteration, (train_x, train_y) in enumerate(train_loader):  # train_x's shape (BATCH_SIZE,1,28,28)
        ...
        loss = criterion(output1, train_y[:, 0]) + criterion(output2, train_y[:, 1])  # cross entropy loss
        ...
        for p in model.parameters():
            ...
            if len(p_size) == 2 and p_size[0] == 2 and p_size[1] == 2:
                p.data.clamp_(0, 1.0)
    model.eval()
    if epoch % 50 == 0:
