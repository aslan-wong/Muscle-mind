users = ["user1", "user2", "user3", "user4", "user5", "user6", "user7", "user8", "user9", "user10", "user11", "user13"];
for i = 1 : length(users)
    get_rms(users(i));
end
function get_rms(user)
    close all;
   ...
   maindirs = [...
    ...
    %     figure('position',position(9,:));plot(data/8);
        ...
        RMS_Data = [RMS_Data, rms(Detrend_Data)];
    %     figure;plot(data/8);hold on;plot(EMG_mask);
    end
    
    fid=fopen('./rms_result_all.txt','a+');%写入文件路径
    for jj=1:length(RMS_Data)
        fprintf(fid,'%.4f\t',RMS_Data(jj)); 
        if mod(jj, 6) == 0
            fprintf(fid,'\r\n');
        end
    end
